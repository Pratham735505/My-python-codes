...
def admin_login():
    ...
    email_label = tk.CTkLabel(wi, text="Email:")
    email_label.grid(row=4, column=7, padx=10, pady=10)
    email_entry = tk.CTkEntry(wi, textvariable=email_var)
    email_entry.grid(row=4, column=8, padx=10, pady=10)

    password_label = tk.CTkLabel(wi, text="Password:")
    password_label.grid(row=5, column=7, padx=10, pady=10)
    password_entry = tk.CTkEntry(wi, textvariable=password_var, show="*")
    password_entry.grid(row=5, column=8, padx=10, pady=10)
    # Creating button for login page
    login_button = tk.CTkButton(wi, text="Login", command=login)
...
def dashboard(name):
    ...
    def add_information():
        ...
        def save():
            ...
            try:
                ...
                success_label = tk.CTkLabel(add_info_window, text="Information saved successfully.", fg="green")
                success_label.pack()
    ...
    log=tk.CTkButton(dashboard_window,label="Logout", command=logout)
    add=tk.CTkButton(dashboard_window,text="Add Information", command=add_information)
    add.pack()
    log.pack()
...
'''
        ...
        if not result:
            ...
            # Showing a success message
            success_label = tk.CTkLabel(root, text="Registration successful.", fg="green")
        ...
        # If user already exists, show an error message
            ...
            error_label = tk.CTkLabel(root, text="User already exists. Please try a different name, email or contact.", fg_color="red")
...
def login_page():
    # Clearing the main window
    global email_label, email_entry, password_entry, password_label
    for widget in root.winfo_children():
        widget.destroy()
    ...
    email = tk.CTkEntry(master=root, width=240, height=32, placeholder_text="Username")
    email.grid(row=8,column=8,pady=12, padx=10)

    password = tk.CTkEntry(master=root, width=240, height=32, placeholder_text="Password", show="*")
    password.grid(row=9,column=8,pady=12, padx=10)
...
def login():
    ...
    if result:
        ...
        cur.execute('select name from users where email=? and password=?', (eml,password))
        name = cur.fetchone()
        dashboard(name[0])
